...
def cosine_similarity(query_heads, key_heads, value_heads, num_heads):
    ...
    flattened_vectors = [
        head.reshape(-1) for head in query_heads
    ]  # Flatten each matrix
    pair_similarities = []
...
def mean_pool(key_or_value_heads, d_model, n_heads, kv_heads, h_dim, min_flag):
    """
    ...
    if min_flag:
        ...
        key_or_value_heads = (
            key_or_value_heads.view(kv_heads, n_heads // kv_heads, h_dim, d_model)
            .min(axis=1)
            .values
        )
    else:
...
class CustomT5SelfAttention(T5Attention):
    def __init__(self, t5_decoder_attention_block, kv_heads: int, min_flag=False):
        ...
        self.has_relative_attention_bias = (
            t5_decoder_attention_block.has_relative_attention_bias
        )
        self.kv_heads = kv_heads
        self.similarity_flag = False  # similarity_flag
        self.grouped_pairs = None

        if self.has_relative_attention_bias:
            self.relative_attention_bias = (
                t5_decoder_attention_block.relative_attention_bias
            )
        ...
        if self.similarity_flag:
            w_q, w_k, w_v, grouped_pairs = cosine_similarity(
                self.q.weight.data, self.k.weight.data, self.v.weight.data, self.n_heads
            )
        ...
        self.k = nn.Linear(
            ...
            out_features=kv_heads * self.key_value_proj_dim,
            bias=False,
        )
        self.k.weight.data = modified_k_weights

        modified_v_weights = mean_pool(
            w_v,
            self.d_model,
            self.n_heads,
            self.kv_heads,
    ...
    def forward(
        ...
        scores = torch.matmul(
            ...
            key_states.repeat_interleave(
                self.n_heads // self.kv_heads, dim=1
            ).transpose(
                3, 2
            ),  # changed here
        ...
        attn_output = unshape(
            torch.matmul(
                attn_weights,
                value_states.repeat_interleave(self.n_heads // self.kv_heads, dim=1),
            )
        )  # (batch_size, seq_length, dim) #changed here
...
def convert_t5_to_mgqa(module, kv_heads: int, inplace: bool = False, min_flag=False):

    out = module if inplace else deepcopy(module)

    # use custom attention blocks in decoder
    for layer in out.decoder.block:
        layer.layer[0].SelfAttention = CustomT5SelfAttention(
            layer.layer[0].SelfAttention, kv_heads, min_flag
        )
        layer.layer[1].EncDecAttention = CustomT5SelfAttention(
            layer.layer[1].EncDecAttention, kv_heads, min_flag
        )
...
if __name__ == "__main__":
    ...
    for kv_heads in [4]:
        for min_flag in [False, True]:
            t5_gqa = convert_t5_to_mgqa(
                t5, kv_heads=kv_heads, inplace=False, min_flag=min_flag
            )
            ...
            text = tokenizer.batch_decode(outputs[0], skip_special_tokens=True)
            print(
                f"Generated text with kv_heads:{kv_heads} and min flag:{min_flag} : {text}"
            )
