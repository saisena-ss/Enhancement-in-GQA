...
def mean_pool(key_or_value_heads, d_model, n_heads, kv_heads, h_dim):
    ...
    key_or_value_heads = key_or_value_heads.view(
        kv_heads, n_heads // kv_heads, h_dim, d_model
    ).mean(axis=1)
    ...
    def forward(
        ...
        def project(hidden_states, proj_layer, key_value_states, past_key_value):
            ...
            if key_value_states is None:
                ...
                hidden_states = shape(
                    torch.einsum("bnd,di->bni", hidden_states, proj_layer.T)
                )  # shape(proj_layer(hidden_states))
            elif past_key_value is None:
                # cross-attn
                # (batch_size, n_heads, seq_length, dim_per_head)
                hidden_states = shape(
                    torch.einsum("bnd,di->bni", key_value_states, proj_layer.T)
                )  # shape(proj_layer(key_value_states))
...
if __name__ == "__main__":
    ...
    for kv_heads in [4]:
        for weight_flag in [True]:
            ...
            text = tokenizer.batch_decode(outputs[0], skip_special_tokens=True)
            print(
                f"Generated text with kv_heads:{kv_heads} and weight flag:{weight_flag} : {text}"
            )
