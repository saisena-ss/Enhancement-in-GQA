...
def train(
    ...
    t5, optimizer,train_dataloader,lr_scheduler = deepspeed.initialize(
        model = t5,
        model_parameters = t5.parameters(),
        # training_data = 
    )
    device = get_accelerator().device_name(t5.local_rank)
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        for batch in train_dataloader:
            ...
            if steps % config.INTERVAL_STEPS == 0:
                ...
                t5.eval()
                # if rank == 0:
                mean_eval_loss_steps, eval_dict_list_steps = validation_loop(
                    t5, tokenizer, metric, eval_dataloader, steps, device,dataset_name
                )
                key_names = eval_dict_list_steps[0].keys()
                average_dict = {k: get_avg(eval_dict_list_steps, k) for k in key_names}
                for k in average_dict.keys():
                    val_rouge_dict_steps[k].append(average_dict[k])
                print(f"Steps: {steps} val rogue {val_rouge_dict_steps}")
                run.log(
                    {
                        f"{logging_name.lower()}_val_epoch_{steps}_" + k: v[0]
                        for k, v in val_rouge_dict_steps.items()
                    }
                )
                # print(rank)
                # if rank == 0:
                ...
                test_rouge_dict_steps = {k: get_avg(test_dict_list, k) for k in key_names}
                if dataset_name=='wmt14':
                    print(f"Epoch: {epoch} BLEU {test_rouge_dict_steps}")
                else:
                    print(f"Epoch: {epoch} test rogue {test_rouge_dict_steps}")
                run.log(
                    {
                        f"{logging_name.lower()}_test_epoch_{steps}_" + k: v
                        for k, v in test_rouge_dict_steps.items()
                    }
                )
                run.log({"Val Steps Loss":mean_eval_loss_steps})
    ...
    with TemporaryDirectory() as tmpdir:
            ...
            ray.train.report(
                metrics=val_rouge_dict, checkpoint=Checkpoint.from_directory(tmpdir)
            )
...
if __name__ == "__main__":
    ...
    trainer = TorchTrainer(
        ...
        scaling_config=ScalingConfig(num_workers=1, use_gpu=True,resource_per_worker={"CPU":4,"GPU":1}),
