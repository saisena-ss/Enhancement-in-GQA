...
def validation_loop(t5, tokenizer, metric, eval_dataloader, step, rank,dataset_name):
    epoch_eval_loss = torch.zeros(2).to(rank)
    eval_dict_list = []
    if rank==0:
    ...
    for eval_batch in eval_dataloader:
        ...
        epoch_eval_loss[0] = eval_loss.item()
        epoch_eval_loss[1] += 1
        ...
        final_metrics= val_rouge_step_metric

    dist.all_reduce(epoch_eval_loss,op=dist.ReduceOp.SUM)
    dist.all_reduce(ddp_metric,op=dist.ReduceOp.SUM)
    if rank == 0:
...
def testing_loop(t5, tokenizer, metric, test_dataloader, rank,dataset_name):
    if rank == 0:
    ...
    for test_batch in test_dataloader:
        ...
        if dataset_name == "wmt14":
            metrics = compute_bleu_metric(test_batch_pred_tensors.cpu(),
                                            test_batch["labels"].cpu(),
                                            tokenizer,
                                            metric,)          
            
        else:
            metrics = compute_metrics(test_batch_pred_tensors.cpu(),
                                      test_batch["labels"].cpu(),
                                      tokenizer,
                                      metric)
...
def train_step(t5,rank,train_dataloader,optimizer,progress_bar,run,tokenizer,metric,eval_dataloader,dataset_name,logging_name,steps):
    ...
    for batch in train_dataloader:
        ...
        loss = outputs.loss
        loss.backward()
        epoch_train_loss[0] = loss.item()
        epoch_train_loss[1] = 1
...
def train(
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        test_dict_list = testing_loop(t5, tokenizer, metric, test_dataloader, rank,dataset_name)
