...
def load_model(checkpoint_path, model_name):
    ...
    if model_name == "GQA":
        ...
        t5_finetuned = convert_t5_to_wgqa(
            t5, kv_heads=4, weight_flag=True, if_random=False
        )
    elif model_name == "SIMGQA":
        t5_finetuned = convert_t5_to_gqa(t5, kv_heads=4, similarity_flag=True)
    elif model_name == "RANDWGQA":
        t5_finetuned = convert_t5_to_wgqa(
            t5, kv_heads=4, weight_flag=True, if_random=True
        )
    elif model_name == "MQA":
...
def validation_loop(t5, tokenizer, metric, eval_dataloader, device):
    ...
    for eval_batch in tqdm(eval_dataloader):
        ...
        eval_batch_pred_tensors = t5.generate(
            eval_batch["input_ids"], max_length=chk_config.MAX_TARGET_LENGTH
        )
...
def testing_loop(t5, tokenizer, metric, test_dataloader, device):
    ...
    for test_batch in tqdm(test_dataloader):
        ...
        test_batch_pred_tensors = t5.generate(
            test_batch["input_ids"], max_length=chk_config.MAX_TARGET_LENGTH
        )
...
def checkpoint_results(run, models_info):
    ...
    def preprocess_function(
        examples,
        max_input_length: int = chk_config.MAX_INPUT_LENGTH,
        max_target_length: int = chk_config.MAX_TARGET_LENGTH,
    ):
        prefix = "summarize: "
    ...
    cnn_data_test = load_dataset(
        "cnn_dailymail", data_dir=data_dir, split=f"test[:{chk_config.PERCENT_DATA}%]"
    )
    cnn_data_val = load_dataset(
        "cnn_dailymail",
        data_dir=data_dir,
        split=f"validation[:{chk_config.PERCENT_DATA}%]",
    )
    ...
    tokenized_datasets_val = cnn_data_val.map(
        ...
        batch_size=chk_config.TOKENIZE_BATCH_SIZE,
    )
    ...
    eval_dataloader = DataLoader(
        ...
        batch_size=chk_config.VAL_BATCH_SIZE,
    ...
    for model_name in [models_info]:
        ...
        if model_name in os.listdir(os.getcwd()):
            ...
            for chk_point in checkpoints:  # iterate through each check point
                ...
                _, _, _, step_type, step_size = chk_point.split(".")[0].split(
                    "_"
                )  # ['_', 't5', 'finetuned', 'steps', '100000']

                t5_finetuned.eval()
                print(f"Started validation for {file_name}")
                mean_eval_loss, eval_dict_list = validation_loop(
                    t5_finetuned, tokenizer, metric, eval_dataloader, device
                )
                ...
                print(
                    f"Model name:{model_name} file_name: {file_name} val rogue {val_rouge_dict}"
                )
                ...
                test_dict_list = testing_loop(
                    t5_finetuned, tokenizer, metric, test_dataloader, device
                )
                key_names = test_dict_list[0].keys()
                test_rouge_dict = {k: get_avg(test_dict_list, k) for k in key_names}

                print(
                    f"Model name: {model_name} file_name:{file_name} test rogue {test_rouge_dict}"
                )
...
if __name__ == "__main__":
    ...
    run = wandb.init(
        project=chk_config.WANDB_PROJECT,
        config={"model": "Val and Test Results"},
        entity=chk_config.WANDB_ENTITY,
        group="Val and Test Results",
    )
