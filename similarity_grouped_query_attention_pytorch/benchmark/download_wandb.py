...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Directory of files to download from wandb"
    )
    ...
    run = wandb.init(
        project=WANDB_PROJECT, config={"type": "model_files"}, entity=WANDB_ENTITY
    )

    artifact = run.use_artifact(
        f"athe_kunal/similarity_gqa/{dir_name}:v0", type="model"
    )
